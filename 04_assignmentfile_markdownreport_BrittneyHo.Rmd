...
agencycount_compare %>%
  ...
  gt() %>%
  tab_header(
    title = "Total Number of People Appointed in Each Agency + Change from Pervious Count"
  ) %>%
  cols_label(
    agency = "Agency",
...
staffcount_current <- staff_data_current %>%
  count(name, name = "current_staff_count")

staffcount_current

staffcount_previous <- staff_data_previous %>%
  count(name, name = "previous_staff_count")

staffcount_previous

staffcount_compare <- left_join(staffcount_current, staffcount_previous, by = "name")
staffcount_compare

#something is weird in this code 
staffcount_compare <- staffcount_compare %>%
  mutate(
    change = staffcount_current - staffcount_previous
  )
